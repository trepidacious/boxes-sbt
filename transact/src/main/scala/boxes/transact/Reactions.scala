...
object Path {
  def apply[T](path : Txn => Box[T])(implicit s: Shelf) = {    
    s.transact{
      implicit txn => {
        val e = path(txn)
        val eVal = e()
        ...
        val r = new PathReaction[T, T](v, (txn: Txn) => Some(path(txn)), null.asInstanceOf[T], new TConverter[T])
        txn.createReaction(r.react(_))
        (v, r)
      }
    }    
  }
 ...
 */
object PathToOption {
  def apply[T](path : Txn => Option[Box[Option[T]]])(implicit s: Shelf) = {
    s.transact{
      implicit txn => {
...
object PathViaOption {
  def apply[T](path: Txn => Option[Box[T]])(implicit s: Shelf) = {
    s.transact{
      implicit txn => {
        val v:Box[Option[T]] = Box(None)
