...
object Path {
  def apply[T](path : Txn => Box[T])(implicit s: Shelf) = boxAndReaction(path)(s)._1
  
  def boxAndReaction[T](path : Txn => Box[T])(implicit s: Shelf) = {    
    s.transact{
      implicit txn => {
        val e = path(txn)
        val eVal = e()
 ...
 */
object PathToOption {
  def apply[T](path : Txn => Option[Box[Option[T]]])(implicit s: Shelf) = boxAndReaction(path)(s)._1

  def boxAndReaction[T](path : Txn => Option[Box[Option[T]]])(implicit s: Shelf) = {
    s.transact{
      implicit txn => {
...
object PathViaOption {
  ...
  def apply[T](path : Txn => Option[Box[T]])(implicit s: Shelf) = boxAndReaction(path)(s)._1
  
  def boxAndReaction[T](path: Txn => Option[Box[T]])(implicit s: Shelf) = {
    s.transact{
      implicit txn => {
        val v:Box[Option[T]] = Box(None)
