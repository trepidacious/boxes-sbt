...
/**
 * View/edit a Box[Int] as the selected index of a ListSelectionModel
 * used with a JTable.
 ...
 */
class ListSelectionIndexModel(v: Box[Option[Int]], setFilter: => Boolean, table:JTable)(implicit s: Shelf) extends ListSelectionModel {
  ...
  private def handleDelegateChange() {
    ...
    if (setFilter) {
      ...
      //actions occur BEFORE we change the box. This gives other swing code
      //a chance to execute BEFORE it receives any additional changes.
      //For example, anything editing an object selected (in a reaction) by
