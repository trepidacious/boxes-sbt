...
*/
class ListSelectionIndicesModel(v: Box[Set[Int]], setFilter: => Boolean, table: JTable)(implicit shelf: Shelf) extends ListSelectionModel {
  ...
  private def handleDelegateChange() {
    ...
    if (setFilter) {
      ...
      SwingView.addUpdate(this,
        shelf.transact(implicit txn => v() = selection)
      )
