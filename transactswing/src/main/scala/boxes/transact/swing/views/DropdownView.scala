...
import boxes.swing.DropdownButton

object DropdownView {
  def apply(v: Box[Ledger], i: Box[Option[Int]], sorting:Boolean = false, minWidth:Int = 250, maxHeight:Int = 300, displayHeader:Boolean = false)(implicit shelf: Shelf) = 
    new DropdownView(v, i, sorting, minWidth, maxHeight, displayHeader)
}

class DropdownView(v: Box[Ledger], i: Box[Option[Int]], sorting:Boolean = false, minWidth:Int = 250, maxHeight:Int = 300, displayHeader:Boolean = false)(implicit shelf: Shelf) extends SwingView {
  ...
  private val selectedString = BoxNow.calc(implicit txn => {
