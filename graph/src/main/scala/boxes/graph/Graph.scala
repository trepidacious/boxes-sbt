...
class GraphBox(fill:Box[Color, _], outline:Box[Color, _], enabled:Box[Boolean, _] = Val(true), action:(Area, GraphSpaces) => Unit, val minSize:Int = 5, val axis: Option[Axis] = None) extends GraphLayer {
...
case class GraphBasic(layers:ListRef[GraphLayer], overlayers:ListRef[GraphLayer], dataArea:Ref[Area], borders:Ref[Borders], highQuality: Ref[Boolean]) extends Graph {}
