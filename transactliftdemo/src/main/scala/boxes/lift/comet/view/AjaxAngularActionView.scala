...
//
////TODO use soemthing like the header to make assignment of action to scope instant - can we just put a script in here? This would mean the buttons could work BEFORE the comet sends its partial update.
...
//  })
//  
//  override def partialUpdates = List(() => JE.JsRaw("angular.element('#" + elementId + "').scope().$apply(function ($scope) {$scope." + v + " = '" + call.guid + "';});"))
//}
//
