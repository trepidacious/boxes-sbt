...
case class LedgerChange(oldValue: Ledger, newValue: Ledger, columnsChanged:Boolean, rowCountChanged:Boolean) extends SingleChange[Ledger]
...
class DirectRecordView[T](fieldName: String)(implicit valueManifest:Manifest[T]) extends RecordView[T] {
  ...
  def fieldClass(field:Int):Class[_] = valueManifest.runtimeClass
  def fieldCount() = 1
}
...
class LensRecordView[T](lenses:Lens[T,_]*) extends RecordView[T] {
  ...
  override def fieldClass(field:Int) = lenses(field).valueManifest.runtimeClass
